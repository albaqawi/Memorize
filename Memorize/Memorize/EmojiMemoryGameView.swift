...
struct EmojiMemoryGameView: View {
    ...
    var body: some View {
        VStack {
            Text("\(viewModel.score)").font(.system(size: 50, weight: .bold, design: .serif))
...
struct CardView: View {
    ...
    //now i can expand this to func to pass a Color or LinearGradient and return
    // LinearGradient if it is not nil or Color if it is only passed
    // remember you must use 2 types T and U in this case
    private func gradiantColor<Element>(for card: Color) -> Element {
        ...
        return gradiantCardColor as! Element
    }
